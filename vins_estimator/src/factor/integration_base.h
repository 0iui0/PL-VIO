...
public:
    ...
    void midPointIntegration(double _dt,
        ...
        result_delta_q = delta_q *
                         Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);   // equation 42
        ...
        if (update_jacobian) {
            ...
            F.block<3, 3>(0, 9) =
                    -0.25 * (delta_q.toRotationMatrix() + result_delta_q.toRotationMatrix()) * _dt * _dt;  // dp_dba
            ...
            F.block<3, 3>(6, 3) = -0.5 * delta_q.toRotationMatrix() * R_a_0_x * _dt +
                                  ...
                                  (Matrix3d::Identity() - R_w_x * _dt) * _dt;  // dv_dtheta
            F.block<3, 3>(6, 6) = Matrix3d::Identity();                   // dv_dv
            F.block<3, 3>(6, 9) =
                    -0.5 * (delta_q.toRotationMatrix() + result_delta_q.toRotationMatrix()) * _dt;    // dv_dba
            F.block<3, 3>(6, 12) =
                    -0.5 * result_delta_q.toRotationMatrix() * R_a_1_x * _dt * -_dt;                 // dv_dbg

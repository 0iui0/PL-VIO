...
void MarginalizationInfo::marginalize() {
    ...
    for (const auto &it : parameter_block_size)    //遍历所有的参数块
    {
        ...
        if (parameter_block_idx.find(it.first) ==
            parameter_block_idx.end())   // 如果这些参数没有添加进parameter_block_idx，把他们也添加进去
        {
...
bool MarginalizationFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++) {
        ...
        Eigen::VectorXd x0 = Eigen::Map<const Eigen::VectorXd>(marginalization_info->keep_block_data[i],
                                                               size);   // marg留下的的prior估计
        if (size != 7)

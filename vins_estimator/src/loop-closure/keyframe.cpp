    ...
    for (auto &it_per_id : estimator.f_manager.feature) {
        ...
        if (it_per_id.start_frame <= WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2) {
            ...
            point_clouds.push_back(
                    estimator.Rs[it_per_id.start_frame] * (qic * pts_i + tic) + estimator.Ps[it_per_id.start_frame]);
        }
    }
}
...
void KeyFrame::PnPRANSAC(vector <cv::Point2f> &measurements_old,
    ...
    if (CV_MAJOR_VERSION < 3)
        ...
        if (CV_MINOR_VERSION < 2)
            solvePnPRansac(pts_3_vector, measurements_old_norm, K, D, rvec, t, true, 100, sqrt(10.0 / 460.0), 0.99,
                           inliers);
        else
